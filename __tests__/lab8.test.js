describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const productButton = await shadow.$('button');
    ...
    expect(await buttonInnerText.jsonValue()).toBe("Remove from Cart");
    await productButton.click();
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < productItems.length; i++){
      const currentShadow = await productItems[i].getProperty('shadowRoot');
      const currentButton = await currentShadow.$('button');
      await currentButton.click();
    }
    // Check to see if the innerText of #cart-count is 20
    let cartCount = await page.$('#cart-count');
    let cartText = await cartCount.getProperty('innerText');
    expect(await cartText.jsonValue()).toBe("20");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < productItems.length; i++){
      const currentShadow = await productItems[i].getProperty('shadowRoot');
      const currentButton = await currentShadow.$('button');
      let buttonInnerText = await currentButton.getProperty('innerText');
      expect(await buttonInnerText.jsonValue()).toBe("Remove from Cart");
    }
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let cartCount = await page.$('#cart-count');
    let cartText = await cartCount.getProperty('innerText');
    expect(await cartText.jsonValue()).toBe("0");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < productItems.length; i++){
      ...
      const currentButton = await currentShadow.$('button');
      let buttonInnerText = await currentButton.getProperty('innerText');
      expect(await buttonInnerText.jsonValue()).toBe("Add to Cart");
    }
